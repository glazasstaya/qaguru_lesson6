...
def test_dark_theme_by_time_and_user_choice():
    ...
    if dark_theme_enabled_by_user == True:
        is_dark_theme = True
    elif dark_theme_enabled_by_user == False or time(hour=6) <= current_time < time(hour=22):
        is_dark_theme = False
    else:
        is_dark_theme = True
...
def open_browser(browser_name):
    actual_result = formated_string(open_browser.__name__, browser_name)
    assert actual_result == "Open Browser [Chrome]"


def go_to_companyname_homepage(page_url):
    actual_result = formated_string(go_to_companyname_homepage.__name__, page_url)
    assert actual_result == "Go To Companyname Homepage [https://companyname.com]"


def find_registration_button_on_login_page(page_url, button_text):
    actual_result =  formated_string(find_registration_button_on_login_page.__name__, page_url, button_text)
